...
class JavaScriptParser(BaseParser):
    """JavaScript/TypeScript parser with tree-sitter support"""
...
class JavaScriptParser(BaseParser):
    ...
    def parse_file(self, file_path: str) -> List[CodeChunk]:
        """Parse JavaScript/TypeScript file and extract functions/classes"""
        try:
    ...
    def _find_definitions(self, node):
        """Recursively find all function/class definitions"""
        definitions = []
    ...
    def _extract_chunk(self, node, source_code: bytes, file_path: str, chunk_index: int) -> Optional[CodeChunk]:
        ...
        if not symbol_name and node.type in ['arrow_function', 'function_expression']:
            symbol_name = f"anonymous_{node.type}"
