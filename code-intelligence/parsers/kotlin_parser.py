"""Kotlin parser for Android apps"""
...
try:
    ...
    logger.warning("tree-sitter-kotlin not available, using fallback")
...
class KotlinParser(BaseParser):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if HAS_TREE_SITTER:
            try:
                self.parser = Parser()
                KOTLIN_LANGUAGE = Language(tskotlin.language(), 'kotlin')
                self.parser.language = KOTLIN_LANGUAGE
                self.language_obj = KOTLIN_LANGUAGE
            except Exception as e:
                logger.error(f"Failed to initialize tree-sitter Kotlin: {e}")
                self.parser = None
        else:
            self.parser = None
    ...
    def parse_file(self, file_path: str) -> List[CodeChunk]:
        """Parse Kotlin file and extract classes/functions"""
        try:
    ...
    def _extract_chunk(self, node, source_code: bytes, file_path: str, chunk_index: int) -> Optional[CodeChunk]:
        ...
        chunk_type = 'class' if 'class' in node.type or 'object' in node.type else 'function'
